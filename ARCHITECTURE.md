...
Essa última verificação pegou um bug real: `useCheckin` guardava o check-in de hoje em estado local por instância do hook, então a atualização otimista feita a partir do modal de check-in não aparecia na tela Hoje (outra instância do mesmo hook). Corrigido movendo esse estado para `store/checkinHojeStore.ts`, compartilhada entre todas as instâncias — documentado no commit correspondente. Fica registrado aqui como prova de que a verificação "rodando" encontrou algo que só typecheck/build não pegariam.
